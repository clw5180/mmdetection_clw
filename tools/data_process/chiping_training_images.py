...
chip_strategy = 'chips_strategy_1333_800_version_4'
...
def myfunc(idx, each_item, number_repeat, share_box_num, share_test_dict, share_lock):
    print('index:', idx)
    # print(each_item)
    ...
    for now_repeat in range(number_repeat):
        ...
        if center_chip:
            ...
            # random chips
            min_w, max_w = min(int((bbox[2] - bbox[0]) / 2), int(window_w - (bbox[2] - bbox[0]) / 2)), max(
                int((bbox[2] - bbox[0]) / 2), int(window_w - (bbox[2] - bbox[0]) / 2))
            min_h, max_h = min(int((bbox[3] - bbox[1]) / 2), int(window_h - (bbox[3] - bbox[1]) / 2)), max(
                int((bbox[3] - bbox[1]) / 2), int(window_h - (bbox[3] - bbox[1]) / 2))
            random_w = random.randint(min_w, max_w)
            random_h = random.randint(min_h, max_h)
            x, y, r, b = center_x - random_w, center_y - random_h, center_x + (window_w - random_w), center_y + (
                        window_h - random_h)
        ...
        for e_box in boxes:
            ...
            if x < e_box[0] < r and y < e_box[1] < b and x < e_box[2] < r and y < e_box[3] < b:
                ...
                share_test_dict[share_box_num.get()] = {
                    'name': each_item['name'][:-4] + '_' + str(idx) + '_chip_' + str(now_repeat) + '.jpg',
                    'image_height': window_h, 'image_width': window_w, 'category': e_box[4],
                    'bbox': {'0': e_box[0], '1': e_box[1], '2': e_box[2], '3': e_box[3]}}

                share_box_num.set(share_box_num.get() + 1)
                print('box_num: ', share_box_num)
                # 释放锁
                share_lock.release()
...
def chiping_training(number_repeat):
    ...
    '''
        ...
        'd'	double	float
    '''
    share_box_num = multiprocessing.Manager().Value("i", 0)

    # 字典声明方式
    share_test_dict = multiprocessing.Manager().dict()
    ...
    p = Pool(12)
    for idx, each_item in enumerate(image_meta):
        p.apply_async(myfunc, args=(idx, each_item, number_repeat, share_box_num, share_test_dict, share_lock))
        #myfunc(idx, each_item, number_repeat)
    p.close()
    p.join()

    print('Number of bboxes:', share_box_num)
    num_images = glob(root+chip_strategy+'/images/*')
    print('Number of chips: ', num_images)
    print('process id:', idx)
    
    save_json_path = root+chip_strategy+"/Annotations/record_all.json"
    mydict = dict(share_test_dict)  # 注意，共享dict无法直接dumps，会报类型错误，必须先转换为普通字典
